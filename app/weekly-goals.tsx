...
type GoalGroup = { title: string; fields: GoalField[] };
    ...
    <ScreenContainer className="px-5 pt-5">
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          <Pressable accessibilityRole="button" accessibilityLabel="חזרה להגדרות" onPress={() => router.back()} style={styles.back}>
            <Text style={styles.backText}>‹ חזרה</Text>
          </Pressable>
          <Text style={styles.eyebrow}>הגדרות מתקדמות</Text>
          <Text style={styles.title}>יעדים שבועיים</Text>
          <Text style={styles.subtitle}>הגדר את היעדים האישיים שיוצגו בפסי ההתקדמות בפרופיל.</Text>
        </View>
        <View style={styles.info}>
          <Text style={styles.infoTitle}>איך זה עובד?</Text>
          <Text style={styles.infoText}>הפרופיל משווה את הנתונים שנצברו השבוע ליעדים שלך. ניתן לשנות את היעדים בכל עת; הם נשמרים במכשיר בלבד.</Text>
        </View>
            ...
            {group.fields.map((field, index) => (
              <View key={field.key} style={[styles.field, index === 0 && styles.fieldFirst]}>
                ...
                <View style={styles.inputRow}>
                  ...
                  <TextInput
                    accessibilityLabel={`יעד ${field.label}`}
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="שמור יעדים שבועיים" onPress={() => void save()} style={styles.primary}>
          <Text style={styles.primaryText}>שמור יעדים</Text>
        </Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="אפס יעדים לברירת המחדל" onPress={() => void reset()} style={styles.secondary}>
          <Text style={styles.secondaryText}>אפס לברירת המחדל</Text>
        </Pressable>
