...
export default function ActiveWorkoutScreen() {
  ...
  const cardioMinutes = activeSession.sets.reduce((sum, set) => sum + (Number(set.reps) || 0), 0);
  const cardioDistance = activeSession.sets.reduce((sum, set) => sum + (Number(set.weight) || 0), 0);
  ...
  return (
    <ScreenContainer className="px-4 pt-4" containerClassName="bg-background">
      ...
      <View style={styles.progressCard}>
        ...
        <Text style={styles.progressText}>{isCardioWorkout ? `${completed} מתוך ${total} מקטעים הושלמו · ${cardioMinutes} דקות · ${cardioDistance.toFixed(1)} ק״מ` : `${completed} מתוך ${total} סטים הושלמו · נפח נוכחי ${Math.round(calculateVolume(activeSession))} ק״ג`}</Text>
