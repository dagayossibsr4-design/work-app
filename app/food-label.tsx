...
const foodSubgroups: FoodSubgroup[] = [
  "עופות", "בשר", "דגים", "גבינות ומוצרי חלב", "ביצים", "קטניות ותחליפים",
  "משקאות חלבון", "אבקות חלבון", "חטיפי חלבון", "חטיפי בריאות ודגנים",
  "לחמים ומאפים", "פסטות ודגנים", "סלטים קנויים", "ממרחים ורטבים",
  "שמנים, אגוזים וזרעים", "חטיפים ומוצרים מוכנים", "פחמימות", "פירות וירקות", "שונות",
];
    ...
    <ScreenContainer className="px-5 pt-5">
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {phase === "success" && !isBusy ? (
          <StatusPanel tone="success" title="התמונה נותחה בהצלחה" copy="בדוק את הערכים למטה מול האריזה לפני שמירה למאגר." />
        ) : null}

        {imageUri ? (
          <View style={styles.previewSection}>
            <View style={styles.previewHeader}>
              <Text style={styles.previewLabel}>התמונה שנבחרה</Text>
              <Text style={styles.previewBadge}>מוכנה לבדיקה</Text>
            </View>
        ...
        {/* שלב 04 */}
        <View style={styles.card}>
          <SectionHeader number="04" title="שיוך למאגר" subtitle="בחר קטגוריה כדי למצוא את המוצר במהירות אחר כך" />
          ...
          <View style={styles.categoryGrid}>
            {foodGroups.map((item) => {
              ...
              return (
                <Pressable
                  key={item}
                  onPress={() => { setGroup(item); setSubgroup("ללא"); }}
          ...
          <Pressable onPress={() => setSubgroupPickerOpen((v) => !v)} style={styles.subgroupTrigger}>
            <View style={styles.subgroupTriggerText}>
              ...
              <Text style={styles.subgroupHint}>אפשר לסנן את המוצר בהמשך</Text>
            </View>
          ...
          {subgroupPickerOpen ? (
            <View style={styles.subgroupPanel}>
              <Pressable
                onPress={() => { setSubgroup("ללא"); setSubgroupPickerOpen(false); }}
                style={[styles.subgroupOption, subgroup === "ללא" && styles.subgroupOptionActive]}
              >
              ...
              {foodSubgroups.map((item) => (
                <Pressable
                  key={item}
                  onPress={() => { setSubgroup(item); setSubgroupPickerOpen(false); }}
                  style={[styles.subgroupOption, subgroup === item && styles.subgroupOptionActive]}
                >
                  <Text style={[styles.subgroupOptionText, subgroup === item && styles.subgroupOptionTextActive]}>
                    {subgroup === item ? "✓ " : ""}{item}
                  </Text>
                </Pressable>
              ))}
          ...
          <View style={styles.assignmentSummary}>
            <Text style={styles.assignmentSummaryLabel}>השמירה תתבצע תחת</Text>
            <Text style={styles.assignmentSummaryValue}>{group}{subgroup === "ללא" ? "" : ` · ${subgroup}`}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  subgroupHint: { color: "#7E8DA4", fontSize: 10, textAlign: "right" },
  ...
  assignmentSummary: { alignItems: "flex-end", gap: 2, borderTopColor: "#2C3B55", borderTopWidth: 1, paddingTop: 11 },
  assignmentSummaryLabel: { color: "#7E8DA4", fontSize: 10, fontWeight: "800" },
  assignmentSummaryValue: { color: "#F5CF75", fontSize: 13, fontWeight: "900", textAlign: "right" },
