...
import {
  ...
  mealTotals,
  cloneMeal,
  clearMealPlanQuantities,
  gramsFromMealQuantity,
...
import {
  ...
  groupForMealFood,
  previewMealConversion,
  recommendEquivalentSwap,
  totalMealFoodValues,
  type EquivalentSwap,
...
export default function MealPlanScreen() {
  ...
  const clearDailyQuantities = () => {
    ...
    setEaten({});
    setEatenHistory((current) => ({ ...current, [selectedDate]: {} }));
    setMealHistoryByDate((current) => ({
      ...current,
      [selectedDate]: { meals: cloneMeals(clearedMeals), eaten: {} },
    }));
    setEditingQuantityKey(null);
    setEditingNutritionKey(null);
    setActiveSwapKey(null);
    setPending(null);
    setMealFoodPickerId(null);
    setMealEditBackup(null);
    setQuantityDraft("");
    setNutritionDraft({ calories: "", protein: "", carbohydrates: "", fats: "" });
    setClearDailyQuantitiesOpen(false);
    setRebalanceMessage("כל הכמויות היומיות אופסו. מבנה הארוחות וקטלוג המזון נשמרו.");
  };
  ...
  const chooseSwap = (
    ...
    const sourceTotals = mealFoodTotals(food);
    const result = recommendEquivalentSwap(sourceTotals, target, target.group);
    if (!result) {
      setRebalanceMessage(
        `לא ניתן להחליף את ${food.name}: אין ערך ${target.group} לחישוב מדויק.`,
      );
      return;
    }
    setPending({
        ...
        return {
          ...
          foods: meal.foods.map((food, foodIndex) =>
            foodIndex !== pending.foodIndex
              ? food
              : buildReplacementFood(pending.target, pending.result, food.id),
          ),
        };
      }),
    );
  ...
  const toggleMealFoodPicker = (meal: Meal) => {
    const isOpen = mealFoodPickerId === meal.id;
    if (isOpen) {
      setMealFoodPickerId(null);
      setMealFoodSearch("");
      setAddFoodGroupFilter(null);
  ...
  const openMealFoodGroup = (meal: Meal, group: FoodGroup) => {
    const isSameOpenGroup = (editingMealId === meal.id || mealFoodPickerId === meal.id) && addFoodGroupFilter === group;
  ...
  const saveMealEdit = () => {
    ...
    setMealEditBackup(null);
    setMealFoodSearch("");
    setRebalanceMessage("השם, הרכיבים והערכים של הארוחה נשמרו.");
  };
  const saveMealAsReusable = (meal: Meal) => {
    const savedMeal = createSavedMeal(meal, {
      id: `saved-meal-${Date.now()}`,
      name: meal.title,
      savedAt: new Date().toISOString(),
    });
    setSavedMeals((current) => upsertSavedMeal(current, savedMeal));
    triggerFavoriteHaptic();
    setRebalanceMessage(
    ...
    setMeals((current) =>
      current.map((meal) =>
        meal.id === targetMealId
          ? normalizeMealsTo100Grams([applySavedMealToSlot(savedMeal, targetMealId)])[0]
          : meal,
      ),
    );
    setExpandedMealIds((current) =>
      current.includes(targetMealId) ? current : [...current, targetMealId],
    );
    triggerFavoriteHaptic();
    setRebalanceMessage(
      `הארוחה "${savedMeal.name}" נטענה לכרטיס הזה עם ${Math.round(savedMeal.totals.calories)} קק״ל.`,
    );
  };
  const removeReusableMeal = (savedMeal: SavedMeal) => {
    setSavedMeals((current) => current.filter((item) => item.id !== savedMeal.id));
    setRebalanceMessage(`הארוחה השמורה "${savedMeal.name}" הוסרה.`);
  };
  ...
  const applyMealConversion = () => {
    ...
    if (!meal) return;
    const rows = previewMealConversion(meal, mealConversionSelection);
    const nextFoods = rows.flatMap((row) => row.replacement ? [{
      ...row.replacement,
      id: `${row.replacement.id}-${Date.now()}`,
    }] : []);
    if (!nextFoods.length) {
      setRebalanceMessage("בחר לפחות חלופה אחת שיש לה ערך מאקרו תקין לפני אישור ההמרה.");
      return;
    }
    const original = totalMealFoodValues(meal.foods);
    const converted = totalMealFoodValues(nextFoods);
    setMeals((current) => current.map((item) => item.id === meal.id ? { ...item, foods: nextFoods } : item));
    setMealConversionId(null);
    setRebalanceMessage(`${meal.title} הומרה. חלבון ${Math.round(original.protein * 10) / 10}→${Math.round(converted.protein * 10) / 10} ג׳ · פחמימות ${Math.round(original.carbohydrates * 10) / 10}→${Math.round(converted.carbohydrates * 10) / 10} ג׳ · שומן ${Math.round(original.fats * 10) / 10}→${Math.round(converted.fats * 10) / 10} ג׳.`);
  };
  const activeMealConversion = mealConversionId
    ? meals.find((meal) => meal.id === mealConversionId) ?? null
    : null;
  const mealConversionPreview = activeMealConversion
    ? previewMealConversion(activeMealConversion, mealConversionSelection)
    : [];
  const conversionOriginalTotals = activeMealConversion
    ? totalMealFoodValues(activeMealConversion.foods)
    : { calories: 0, protein: 0, carbohydrates: 0, fats: 0 };
  const conversionPreviewTotals = totalMealFoodValues(
    mealConversionPreview.flatMap((row) => row.replacement ? [row.replacement] : []),
  );
  return (
    <ScreenContainer className="px-5 pt-5" containerClassName="bg-background">
      ...
      >
        ...
        >
          {displayedMeals.map((meal, mealIndex) => {
            ...
            const isMealFoodPickerOpen = mealFoodPickerId === meal.id;
            ...
            return (
              ...
              >
                ...
                {isMealExpanded ? (
                  <View style={styles.mealFoodEditor}>
                    ...
                    {meal.foods.map((food, foodIndex) => {
                      const macroGroup = groupForMealFood(food);
                            ...
                            <>
                              <Pressable
                                onPress={() =>
                                  swapOpen
                                    ? setActiveSwapKey(null)
                                    : openSwap(meal.id, food.id, macroGroup)
                                }
                                style={[
                              ...
                              {swapOpen ? (
                                <View style={styles.swapArea}>
                                  ...
                                  <NutritionCategoryBrowser
                                    foods={mealFoods.filter((item) => item.name !== food.name)}
                                    groups={[macroGroup]}
                                    selectedGroup={swapGroup as FoodGroup | null}
                                    selectedSubgroup={swapSubgroup ?? "ללא"}
                                    onSelectGroup={(group) => setSwapGroup(group as ConversionGroup)}
                                    onClearGroup={() => {
                                      setSwapGroup(null);
                                      setSwapSubgroup(null);
                                      setConversionSearch("");
                                    }}
                                    onSelectSubgroup={(subgroup) => setSwapSubgroup(subgroup === "ללא" ? null : subgroup)}
                                    onSelectFood={(item) => chooseSwap(mealIndex, foodIndex, conversionFoodFromFoodItem(item))}
                                    productSearch={conversionSearch}
                                    onProductSearchChange={setConversionSearch}
                                    productFilter={(item) => item.name !== food.name}
                                    productActionText="בחר חלופה"
                                    productSearchPlaceholder={`חפש חלופה בתוך הקטגוריה`}
                                    allowGroupCollapse
                                    initiallyCollapsed
                                  />
                ...
                <View style={styles.mealActions}>
                  ...
                  >
                    <Text style={styles.saveReusableMealText}>שמור כארוחה קבועה</Text>
                    <Text style={styles.saveReusableMealHint}>כל הרכיבים · קלוריות · מאקרו</Text>
                  </Pressable>
                  {savedMeals.length ? (
                    <View style={styles.reusableMealsPanel}>
                      <Text style={styles.reusableMealsTitle}>טען ארוחה שמורה</Text>
                      <Text style={styles.reusableMealsHint}>הבחירה תחליף את תוכן הכרטיס הנוכחי בלבד.</Text>
                      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.reusableMealsList}>
                        {savedMeals.map((savedMeal) => (
                          <View key={savedMeal.id} style={styles.reusableMealCard}>
                            <Pressable
                              accessibilityRole="button"
                              accessibilityLabel={`טען את ${savedMeal.name}`}
                              onPress={() => loadReusableMeal(meal.id, savedMeal)}
                              style={({ pressed }) => [styles.reusableMealLoad, pressed && styles.reusableMealLoadPressed]}
                            >
                              <Text numberOfLines={1} style={styles.reusableMealName}>{savedMeal.name}</Text>
                              <Text style={styles.reusableMealCalories}>{Math.round(savedMeal.totals.calories)} קק״ל</Text>
                              <Text style={styles.reusableMealMacros}>ח׳ {Math.round(savedMeal.totals.protein)} · פ׳ {Math.round(savedMeal.totals.carbohydrates)} · ש׳ {Math.round(savedMeal.totals.fats)}</Text>
                            </Pressable>
                            <Pressable
                              accessibilityRole="button"
                              accessibilityLabel={`הסר את ${savedMeal.name} מהארוחות השמורות`}
                              onPress={() => removeReusableMeal(savedMeal)}
                              style={({ pressed }) => [styles.reusableMealRemove, pressed && styles.reusableMealRemovePressed]}
                            >
                              <Text style={styles.reusableMealRemoveText}>הסר</Text>
                            </Pressable>
                          </View>
                        ))}
                      </ScrollView>
                  ...
                  <View style={styles.addFoodRow}>
                    <Pressable
                      accessibilityRole="button"
                      accessibilityState={{ expanded: isMealFoodPickerOpen }}
                      accessibilityLabel={isMealFoodPickerOpen ? `סגור קטלוג הוספת רכיב ל${meal.title}` : `פתח קטלוג הוספת רכיב ל${meal.title}`}
                      onPress={() => toggleMealFoodPicker(meal)}
                      style={({ pressed }) => [
                        styles.addFoodToggle,
                        isMealFoodPickerOpen && styles.addFoodToggleActive,
                        pressed && styles.addFoodTogglePressed,
                      ]}
                    >
                      <Text style={[styles.addFoodToggleText, isMealFoodPickerOpen && styles.addFoodToggleTextActive]}>
                        {isMealFoodPickerOpen ? "הסתר קטלוג −" : "הוסף רכיב +"}
                      </Text>
                    </Pressable>
                    {isMealFoodPickerOpen ? (
                      <NutritionCategoryBrowser
                        foods={mealFoods}
                        selectedGroup={addFoodGroupFilter}
                        selectedSubgroup={addFoodSubgroupFilter ?? "ללא"}
                        onSelectGroup={(group) => {
                          openMealFoodGroup(meal, group);
                          setRebalanceMessage(`נפתחה בחירת ${group} ב${meal.title}. בחר תת־קטגוריה ומוצר.`);
                        }}
                        onClearGroup={() => {
                          setAddFoodGroupFilter(null);
                          setAddFoodSubgroupFilter(null);
                        ...
                        onSelectSubgroup={(subgroup) => {
                          setAddFoodSubgroupFilter(subgroup === "ללא" ? null : subgroup);
                          setMealFoodSearch("");
                        }}
                  ...
                  {editingMealId === meal.id ? (
                    <>
                      ...
                      >
                        <Text style={styles.mealSaveText}>שמור שם ושינויים</Text>
                      </Pressable>
                      <Pressable
          ...
          <View style={styles.mealConversionBackdrop}>
            <View style={styles.mealConversionModal}>
              <View style={styles.mealConversionHeader}>
                <View>
                  <Text style={styles.mealConversionModalTitle}>המרת ארוחה מדויקת</Text>
                  <Text style={styles.mealConversionModalHint}>הכמות מחושבת לפי המאקרו המרכזי של כל קבוצה; אין בחירה אקראית.</Text>
                </View>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.mealConversionScroll}>
                {(['חלבון', 'פחמימה', 'שומן'] as ConversionGroup[]).map((group) => {
                  const preview = mealConversionPreview.find((row) => row.group === group);
      ...
      const selectedSubgroup = mealConversionSubgroups[group] ?? null;
      const subgroupFoods = selectedSubgroup
        ? groupFoods.filter((food) => food.subgroup === selectedSubgroup).sort((a, b) => (a.calories - b.calories) || a.name.localeCompare(b.name, "he"))
        : [];
                  return (
                    <View key={group} style={styles.mealConversionGroup}>
                      ...
                      <View style={styles.mealConversionChoices}>
                        ...
                        {groupSubgroups.map((subgroup) => <Pressable key={`${group}-${subgroup}`} onPress={() => { setMealConversionSubgroups((current) => ({ ...current, [group]: current[group] === subgroup ? null : subgroup })); setMealConversionSelection((current) => ({ ...current, [group]: null })); }} style={[styles.mealConversionSubgroupChoice, selectedSubgroup === subgroup && styles.mealConversionSubgroupChoiceActive]}>
                      ...
                      {preview?.replacement && replacementTotals ? (
                        <View style={styles.mealConversionPreview}>
                          ...
                          <Text style={styles.mealConversionPreviewText}>קלוריות {Math.round(sourceTotals.calories)} → {Math.round(replacementTotals.calories)} · חלבון {Math.round(sourceTotals.protein * 10) / 10} → {Math.round(replacementTotals.protein * 10) / 10} · פחמימות {Math.round(sourceTotals.carbohydrates * 10) / 10} → {Math.round(replacementTotals.carbohydrates * 10) / 10} · שומן {Math.round(sourceTotals.fats * 10) / 10} → {Math.round(replacementTotals.fats * 10) / 10}</Text>
                        </View>
                      ) : <Text style={styles.mealConversionSelected}>{mealConversionSelection[group] ? "אין יעד מאקרו בקבוצה זו להמרה." : "הקבוצה לא תיכלל בארוחה המומרת."}</Text>}
                    </View>
                  );
                })}
                <View style={styles.mealConversionTotals}>
                  <Text style={styles.mealConversionTotalsTitle}>סיכום לפני אישור</Text>
                  <Text style={styles.mealConversionTotalsText}>קלוריות {Math.round(conversionOriginalTotals.calories)} → {Math.round(conversionPreviewTotals.calories)} · חלבון {Math.round(conversionOriginalTotals.protein * 10) / 10} → {Math.round(conversionPreviewTotals.protein * 10) / 10} · פחמימות {Math.round(conversionOriginalTotals.carbohydrates * 10) / 10} → {Math.round(conversionPreviewTotals.carbohydrates * 10) / 10} · שומן {Math.round(conversionOriginalTotals.fats * 10) / 10} → {Math.round(conversionPreviewTotals.fats * 10) / 10}</Text>
                </View>
              </ScrollView>
              <View style={styles.mealConversionActions}>
                <Pressable onPress={() => setMealConversionId(null)} style={styles.mealConversionCancel}><Text style={styles.mealConversionCancelText}>ביטול</Text></Pressable>
                <Pressable disabled={!mealConversionPreview.some((row) => row.replacement)} onPress={applyMealConversion} style={[styles.mealConversionApply, !mealConversionPreview.some((row) => row.replacement) && styles.disabledAction]}><Text style={styles.mealConversionApplyText}>אשר והחלף</Text></Pressable>
              </View>
            </View>
          </View>
        </Modal>
        ...
        <Modal
          transparent
          visible={clearDailyQuantitiesOpen}
          animationType="fade"
          onRequestClose={() => setClearDailyQuantitiesOpen(false)}
        >
          <View style={styles.clearDailyQuantitiesBackdrop}>
            <View style={styles.clearDailyQuantitiesModal}>
              ...
              <View style={styles.clearDailyQuantitiesActions}>
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel="ביטול ניקוי הכמויות"
                  onPress={() => setClearDailyQuantitiesOpen(false)}
                  style={({ pressed }) => [styles.clearDailyQuantitiesCancel, pressed && styles.swapButtonPressed]}
                >
                  <Text style={styles.clearDailyQuantitiesCancelText}>ביטול</Text>
                </Pressable>
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel="אישור ניקוי כל הכמויות"
                  onPress={clearDailyQuantities}
                  style={({ pressed }) => [styles.clearDailyQuantitiesConfirm, pressed && styles.swapButtonPressed]}
                >
                  <Text style={styles.clearDailyQuantitiesConfirmText}>נקה הכול</Text>
                </Pressable>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  clearDailyButtonText: { color: "#FFD7DC", fontWeight: "900", fontSize: 12 },
  rebalanceMessage: {
    color: "#B8CBE0",
    fontSize: 11,
    textAlign: "right",
  ...
  waterQuickRow: { flexDirection: "row-reverse", flexWrap: "nowrap", gap: 5, width: "100%" },
  waterQuickButton: { flex: 1, minWidth: 0, minHeight: 54, borderRadius: 10, backgroundColor: "#203A63", borderColor: "#8A6B20", borderWidth: 1, alignItems: "center", justifyContent: "center", paddingVertical: 5, paddingHorizontal: 2 },
  ...
  waterQuickButtonText: { color: "#D9E2EF", fontSize: 8, fontWeight: "900", writingDirection: "rtl" },
  waterQuickAmount: { color: "#F7F9FC", fontSize: 9, fontWeight: "900", writingDirection: "rtl" },
  ...
  clearDailyQuantitiesBackdrop: { flex: 1, backgroundColor: "rgba(3, 8, 20, 0.78)", justifyContent: "center", padding: 20 },
  clearDailyQuantitiesModal: { width: "100%", backgroundColor: "#17233A", borderColor: "#F16B7A", borderWidth: 1, borderRadius: 18, padding: 18, gap: 12 },
  clearDailyQuantitiesTitle: { color: "#FFD7DC", fontSize: 19, fontWeight: "900", textAlign: "right", writingDirection: "rtl" },
  clearDailyQuantitiesText: { color: "#D9E2EF", fontSize: 12, lineHeight: 19, textAlign: "right", writingDirection: "rtl" },
  clearDailyQuantitiesHint: { color: "#F5D27A", fontSize: 11, lineHeight: 17, textAlign: "right", writingDirection: "rtl" },
  clearDailyQuantitiesActions: { flexDirection: "row-reverse", gap: 8, marginTop: 4 },
  clearDailyQuantitiesCancel: { flex: 1, borderColor: "#5B7694", borderWidth: 1, borderRadius: 10, minHeight: 46, alignItems: "center", justifyContent: "center" },
  clearDailyQuantitiesCancelText: { color: "#D9E2EF", fontWeight: "800", fontSize: 12 },
  clearDailyQuantitiesConfirm: { flex: 1, backgroundColor: "#D94A5B", borderColor: "#FF9BA7", borderWidth: 1, borderRadius: 10, minHeight: 46, alignItems: "center", justifyContent: "center" },
  clearDailyQuantitiesConfirmText: { color: "#240B11", fontWeight: "900", fontSize: 12 },
  ...
  saveReusableMealHint: { color: "#253653", fontSize: 10, fontWeight: "800", textAlign: "right", writingDirection: "rtl" },
  ...
  reusableMealCalories: { color: "#F5D27A", fontSize: 11, fontWeight: "900", textAlign: "right", writingDirection: "rtl" },
  reusableMealMacros: { color: "#AAB7C8", fontSize: 9, fontWeight: "800", textAlign: "right", writingDirection: "rtl" },
  reusableMealRemove: { borderTopColor: "#3D587C", borderTopWidth: 1, paddingVertical: 7, alignItems: "center" },
  reusableMealRemovePressed: { backgroundColor: "#3B2430", opacity: 0.9 },
  reusableMealRemoveText: { color: "#FB7185", fontSize: 10, fontWeight: "900" },
  ...
  mealConversionScroll: { gap: 10, paddingBottom: 4 },
  ...
  mealConversionPreviewText: { color: "#D9EEFF", fontSize: 9, fontWeight: "700", textAlign: "right", lineHeight: 15, writingDirection: "rtl" },
  mealConversionTotals: { backgroundColor: "#0B1224", borderColor: "#F5B72C", borderWidth: 1, borderRadius: 10, padding: 10, gap: 4 },
  mealConversionTotalsTitle: { color: "#F5D27A", fontSize: 11, fontWeight: "900", textAlign: "right", writingDirection: "rtl" },
  mealConversionTotalsText: { color: "#F7F9FC", fontSize: 9, fontWeight: "800", textAlign: "right", lineHeight: 15, writingDirection: "rtl" },
