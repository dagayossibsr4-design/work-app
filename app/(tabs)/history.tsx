...
import { calculateVolume, sortWorkoutSessionsNewestFirst, useWorkoutStore, type WorkoutSession } from "@/lib/workout-store";
      ...
      <FlatList
        ...
        ListHeaderComponent={<>
          ...
          {orderedSessions.length > 0 && <View style={styles.focusCard}>
            ...
            {selected && <SessionDetail session={selected} template={templates.find((item) => item.id === selected.templateId) ?? getTemplate(selected.templateId)} openInEditMode={edit === "1"} />}
          </View>}
        </>}
...
function SessionDetail({ session, template, openInEditMode = false }: { session: WorkoutSession; template: ReturnType<typeof getTemplate>; openInEditMode?: boolean }) {
  ...
  const withDate = (iso: string, date: string) => { if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return iso; const current = new Date(iso); const next = new Date(`${date}T${String(current.getHours()).padStart(2, "0")}:${String(current.getMinutes()).padStart(2, "0")}:00`); return Number.isNaN(next.getTime()) ? iso : next.toISOString(); };
  const [editing, setEditing] = useState(openInEditMode);
  const [draft, setDraft] = useState(session);
  useEffect(() => { setDraft(session); setEditing(openInEditMode); }, [session, openInEditMode]);
  ...
  const updateDraftSet = (setId: string, patch: { weight?: string; reps?: string; completed?: boolean; note?: string; restSeconds?: number }) => setDraft((current) => ({ ...current, sets: current.sets.map((set) => set.id === setId ? { ...set, ...patch } : set) }));
  const save = () => { if (!/^\d{4}-\d{2}-\d{2}$/.test(toDateInput(draft.startedAt))) return; updateSession(session.id, { startedAt: draft.startedAt, sets: draft.sets }); setEditing(false); };
