...
import { findNodeHandle, Pressable, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
...
export default function EditorScreen() {
  ...
  const [selectedId, setSelectedId] = useState<WorkoutId>((templateId as WorkoutId | undefined) ?? templates[0]?.id ?? "push1");
  ...
  const templateCardRef = useRef<View>(null);
  ...
  useEffect(() => {
    ...
    // waits a beat for that layout pass to settle before measuring.
    const timer = setTimeout(() => {
      const scrollNode = findNodeHandle(scrollRef.current);
      const target = templateCardRef.current as unknown as { measureLayout?: (node: number, onSuccess: (x: number, y: number) => void, onFail?: () => void) => void } | null;
  ...
  return <ScreenContainer className="px-5 pt-5" containerClassName="bg-background">
    ...
    <View style={styles.content}>
      ...
      <View ref={templateCardRef} style={styles.templateCard}>
