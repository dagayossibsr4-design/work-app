    ...
    <View style={styles.modalBackdrop}>
      <View style={styles.myProgramsModal}>
        ...
        {availablePrograms.length ? <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.myProgramsContent}>
              <View style={styles.selectedProgramsSummary}><Text style={styles.selectedProgramsSummaryTitle}>התוכניות שנבחרו</Text>{availablePrograms.map((item) => <View key={`selected-summary-${item.id}`} style={styles.selectedProgramSummaryCard}><Text style={styles.selectedProgramSummaryTitle}>{item.program?.title ?? item.template.name}</Text><Text style={styles.selectedProgramSummaryMeta}>{(item.templates ?? [item.template]).map((template) => template.name).join(" · ")}</Text></View>)}<Text style={styles.selectedProgramsSummaryHint}>בחר יום למטה כדי לשבץ אחד מאימוני התוכנית.</Text></View>
              {days.map((day, index) => {
            const date = new Date(`${weekStart}T12:00:00Z`);
            date.setUTCDate(date.getUTCDate() + index);
            const dateKey = date.toISOString().slice(0, 10);
