...
export default function HomeScreen() {
  ...
  useEffect(() => {
    void AsyncStorage.getItem(selectedMethodStorageKey).then((stored) => {
      if (stored) setSelectedMethodId(stored);
    });
  ...
  const customMethods: TrainingMethod[] = (templates || [])
    .filter((template: WorkoutTemplate) => template.id.startsWith("custom-"))
    .map((template: WorkoutTemplate) => ({
      ...
      icon: (template.icon as IconSymbolName) || "dumbbell.fill",
    }));
  ...
  const selectedDayTemplate = (templates || []).find((template: WorkoutTemplate) => template.id === selectedDayId) ?? getTemplate(selectedDayId) ?? (templates || []).find((template: WorkoutTemplate) => template.id === selectedMethod.templateIds[0]);

  const filteredCustomExercises = exerciseLibrary
    .filter((item) => `${item.name} ${item.englishName} ${(item.aliases ?? []).join(" ")}`.toLowerCase().includes(customSearch.toLowerCase()))
    .slice(0, 14);

  const filteredReplacementExercises = exerciseLibrary
    .filter((item) => `${item.name} ${item.englishName} ${(item.aliases ?? []).join(" ")}`.toLowerCase().includes(replacementSearch.toLowerCase()))
    .slice(0, 8);

  const completedSets = (sessions || []).reduce((sum: number, session: any) => sum + session.sets.filter((set: any) => set.completed).length, 0);
  const last = useMemo(() => sortWorkoutSessionsNewestFirst(sessions || [])[0], [sessions]);
  const previousSessionForPreview = previewTemplate ? (sessions || []).find((session: any) => session.templateId === previewTemplate.id && Boolean(session.finishedAt)) : undefined;
  ...
  const openPreview = (id: WorkoutId) => {
    setPreviewTemplate((templates || []).find((template: WorkoutTemplate) => template.id === id) ?? getTemplate(id));
  ...
  const applyAutoProgress = () => {
    ...
    let weightSets = 0;
    let repSets = 0;
    updatePreview((template) => ({ ...template, exercises: template.exercises.map((exercise) => ({ ...exercise, sets: exercise.sets.map((set, setIndex) => { const previous = previousSessionForPreview?.sets.find((candidate: any) => candidate.exerciseId === exercise.id && candidate.setNumber === setIndex + 1); const progress = calculateFivePercentProgress(previous); if (!progress) return set; updatedSets += 1; if (progress.mode === "weight") weightSets += 1; else repSets += 1; return { ...set, target: progress.mode === "reps" ? progress.value : set.target, suggestedWeight: progress.mode === "weight" ? progress.value : set.suggestedWeight, note: progress.mode === "weight" ? `משקל יעד: ${progress.value} ק״ג` : set.note }; }) })) }));
  ...
  const finishPreview = (saveChanges: boolean) => {
    if (!previewTemplate) return;
    if (saveChanges) updateTemplate(previewTemplate.id, previewTemplate);
  ...
  const switchMethod = (methodId: string) => {
    if (methodId === "cardio") {
      ...
      void AsyncStorage.setItem(selectedMethodStorageKey, methodId);
      setIsCardioPickerOpen(true);
      return;
    }
    ...
    setSelectedMethodId(methodId);
    void AsyncStorage.setItem(selectedMethodStorageKey, methodId);
    setTimeout(() => setIsSwitchingMethod(false), 180);
  };

  const selectTrainingDay = (methodId: string, templateId: WorkoutId) => {
    setSelectedDayByMethod((current) => {
      const next = { ...current, [methodId]: templateId };
      void AsyncStorage.setItem(selectedDayStorageKey, JSON.stringify(next));
      return next;
    });
  };
        ...
        <View style={styles.header}>
          ...
          <View style={styles.titleBlock}>
            ...
            <Text style={styles.title} numberOfLines={1}>
              {displayName ? `שלום ${displayName}!` : "מוכנים לעבוד?"}
            </Text>
            <Text style={styles.subtitle}>{hydrated ? (displayName ? "מסונכרן לענן · כל סט נשמר מיד" : "כל סט נשמר מיד במכשיר") : "טוען את היומן שלך…"}</Text>
            <Text testID="home-build-stamp" style={styles.buildStamp}>גרסת התקנה {Constants.expoConfig?.version ?? "לא ידועה"} · Android build {Constants.expoConfig?.android?.versionCode ?? "לא ידוע"}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  webCardioPicker: { position: "absolute", top: 0, left: 0, right: 0, zIndex: 999, maxHeight: "88%", overflow: "scroll", backgroundColor: "#101A30", borderColor: "#5278A8", borderWidth: 1, borderRadius: 18, padding: 15, gap: 10 },
  cardioPickerModal: { maxHeight: "88%", backgroundColor: "#101B31", borderTopLeftRadius: 26, borderTopRightRadius: 26, borderWidth: 1, borderColor: "#334155", padding: 18 },
  cardioPickerContent: { paddingBottom: 20 },
  cardioOption: { position: "relative", flexDirection: "row-reverse", alignItems: "center", gap: 12, borderWidth: 1, borderRadius: 16, backgroundColor: "#16233A", padding: 14, marginBottom: 10, minHeight: 82 },
  cardioOptionIcon: { width: 52, height: 52, borderRadius: 16, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  cardioOptionText: { flex: 1, alignItems: "flex-end" },
  cardioOptionTitle: { color: "#F7F9FC", fontSize: 16, fontWeight: "900", textAlign: "right" },
  cardioOptionSubtitle: { color: "#AAB7C8", fontSize: 11, textAlign: "right", marginTop: 3 },
  cardioOptionAction: { fontSize: 11, fontWeight: "900", marginTop: 7 },
  cardioMoreCard: { backgroundColor: "#0B1224", borderRadius: 14, padding: 14, marginTop: 2 },
  cardioMoreTitle: { color: "#F5B72C", fontSize: 13, fontWeight: "900", textAlign: "right" },
  cardioMoreText: { color: "#AAB7C8", fontSize: 11, lineHeight: 17, textAlign: "right", marginTop: 5 },
