...
function MyProgramsModal({ visible, selectedPrograms, onClose }: { visible: boolean; selectedPrograms: SelectedProgramItem[]; onClose: () => void }) {
  ...
  const availableTemplates = Array.from(new Map(availablePrograms.flatMap((item) => item.templates ?? [item.template]).map((template) => [template.id, template])).values());
  ...
  const assignToDay = async (date: string, template: WorkoutTemplate) => {
    const next = await assignWorkoutTemplateToDate(date, template, isCardioWorkoutTemplate(template.id) ? "cardio" : "workout");
    ...
    <View style={styles.modalBackdrop}>
      <View style={styles.myProgramsModal}>
        ...
        {availablePrograms.length ? <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.myProgramsContent}>
            ...
            const assigned = availableTemplates.find((template) => template.id === assignedId);
            return <View key={dateKey} style={styles.assignmentCard}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.assignmentChoices}>
                {availableTemplates.map((template) => <Pressable key={`${dateKey}-${template.id}`} accessibilityRole="button" accessibilityState={{ selected: assignedId === template.id }} onPress={() => void assignToDay(dateKey, template)} style={[styles.assignmentChoice, assignedId === template.id && { backgroundColor: `${template.accent}33`, borderColor: template.accent }]}><Text style={[styles.assignmentChoiceText, assignedId === template.id && { color: template.accent }]}>{template.name}</Text></Pressable>)}
              </ScrollView>
            </View>;
          })}
